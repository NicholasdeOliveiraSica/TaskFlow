    ...
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 glass-card rounded-2xl p-4 border border-slate-800/80 mb-5 text-xs text-slate-300">
      ...
      <div className="flex items-center gap-2">
        ...
        >
          ...
          <option value="all">Tudo (Infinite Scroll)</option>
        </select>
      </div>
      ...
      <div className="text-slate-400 font-medium text-center sm:text-left">
        {isInfiniteScroll ? (
          <span className="flex items-center gap-1.5 justify-center sm:justify-start">
            ...
            <span>
              Carregados <strong className="text-slate-200 font-semibold">{displayedItemsCount}</strong> de{' '}
              <strong className="text-slate-200 font-semibold">{totalItems}</strong> itens (20 por vez)
            </span>
          </span>
        ) : (
          <span>
            Mostrando <strong className="text-slate-200 font-semibold">{fromItem}</strong> a{' '}
            <strong className="text-slate-200 font-semibold">{toItem}</strong> de{' '}
            <strong className="text-slate-200 font-semibold">{totalItems}</strong> tarefas
